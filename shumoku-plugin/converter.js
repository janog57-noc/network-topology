 ...
 * NetworkGraph compatible with Shumoku.
 */
...
export async function generateShumokuGraph(client, options = {}) {
  ...
  for (const lag of lags) {
    const id =
      safeString(lag.id) || `${safeString(lag.device?.name)}-${safeString(lag.name)}`
    createdNodeIds.add(id)
    nodes.push({
      id,
      ...
      for (const vci of vc.vci) {
        ...
        if (!createdNodeIds.has(physicalPortId)) {
          ...
          nodes.push({
            id: physicalPortId,
            label: [
              `<b>${safeString(vci.physicalPort.name)}</b>`,
              safeString(vci.physicalPort.device),
            ],
            shape: 'rounded',
            type: 'l3-switch',
          })
  ...
  for (const vc of vcs) {
    ...
    // Create links
    if (vcLinks.length === 2) {
      // If only 2 connections, create direct link without VC node
